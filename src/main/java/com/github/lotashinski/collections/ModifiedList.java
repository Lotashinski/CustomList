...
abstract class ModifiedList<T> extends Modified<T> implements List<T> {
	...
	public int indexOf(Object o) {
		for(int i = 0; i < size(); ++i) {
			...
			if (o == tmp) return i;
			if (o != null && o.equals(tmp)) return i;
		}
	...
	public boolean equals(Object obj) {
	    ...
	    while(currentIterator.hasNext() && otherIterator.hasNext()) {
	    	...
	    	if (ce == null && oe == null) continue;
	    	if (ce != null && ! ce.equals(oe)) return false;
	    }
	...
	private class CustomListiterator<E> extends Modified<E> implements ListIterator<E> {
		...
		public E next() {
			try {
				checkModifications(main);
				if (ref == main.size()) throw new NoSuchElementException();
		...
		public E previous() {
			try {
				...
				if (ref < 0) throw new NoSuchElementException();
